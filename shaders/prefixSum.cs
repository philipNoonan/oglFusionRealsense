...
const uint items_per_work_group = gl_WorkGroupSize.x * 2;

shared uint fastTempArr[items_per_work_group];
...
void forEveryGroup()
{
    ...
    uint indexMultiplierDueToDepth = 1;
    for (int dataPairs = int(items_per_work_group) >> 1; dataPairs > 0; dataPairs >>= 1)
    {
        barrier();
    ...
    for (int dataPairs = 1; dataPairs < int(items_per_work_group); dataPairs *= 2)
    {
        barrier();
