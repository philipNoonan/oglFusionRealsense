...
void gFusion::initTextures()
{
	...
	{
		m_textureDepth = createTexture(GL_TEXTURE_2D, 3, m_depth_width, m_depth_height, 1, GL_R16);
...
void gFusion::depthToVertex(uint16_t * depthArray)
{
	...
	if (depthArray != NULL) 
	{
		glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_depth_width, m_depth_height, GL_RED, GL_UNSIGNED_SHORT, depthArray);
	}
	glGenerateMipmap(GL_TEXTURE_2D);
...
bool gFusion::TrackSDF() {
	...
	//	int level = 0;
		for (int iteration = 0; iteration < configuration.iterations[level]; iteration++)
		{
			...
			dC_icp = dC_icp + (double(iteration) * 0.1)*Eigen::MatrixXd::Identity(6, 6);
